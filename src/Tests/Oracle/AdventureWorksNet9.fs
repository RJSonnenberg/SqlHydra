// This code was generated by `SqlHydra.Oracle` -- v2.8.2.
...
type HydraReader(reader: Oracle.ManagedDataAccess.Client.OracleDataReader) =
    ...
    member __.``OT.CONTACTS`` = OT.Readers.CONTACTSReader(reader, buildGetOrdinal typeof<OT.CONTACTS>)
    member __.``OT.COUNTRIES`` = OT.Readers.COUNTRIESReader(reader, buildGetOrdinal typeof<OT.COUNTRIES>)
    member __.``OT.CUSTOMERS`` = OT.Readers.CUSTOMERSReader(reader, buildGetOrdinal typeof<OT.CUSTOMERS>)
    member __.``OT.EMPLOYEES`` = OT.Readers.EMPLOYEESReader(reader, buildGetOrdinal typeof<OT.EMPLOYEES>)
    member __.``OT.INVENTORIES`` = OT.Readers.INVENTORIESReader(reader, buildGetOrdinal typeof<OT.INVENTORIES>)
    member __.``OT.LOCATIONS`` = OT.Readers.LOCATIONSReader(reader, buildGetOrdinal typeof<OT.LOCATIONS>)
    member __.``OT.ORDERS`` = OT.Readers.ORDERSReader(reader, buildGetOrdinal typeof<OT.ORDERS>)
    member __.``OT.ORDER_ITEMS`` = OT.Readers.ORDER_ITEMSReader(reader, buildGetOrdinal typeof<OT.ORDER_ITEMS>)
    member __.``OT.PRODUCTS`` = OT.Readers.PRODUCTSReader(reader, buildGetOrdinal typeof<OT.PRODUCTS>)
    member __.``OT.PRODUCT_CATEGORIES`` = OT.Readers.PRODUCT_CATEGORIESReader(reader, buildGetOrdinal typeof<OT.PRODUCT_CATEGORIES>)
    member __.``OT.REGIONS`` = OT.Readers.REGIONSReader(reader, buildGetOrdinal typeof<OT.REGIONS>)
    member __.``OT.WAREHOUSES`` = OT.Readers.WAREHOUSESReader(reader, buildGetOrdinal typeof<OT.WAREHOUSES>)
...
module HydraBuilders =
    ...
    let selectTask ct =
        selectTask<'Selected, 'Mapped, Oracle.ManagedDataAccess.Client.OracleDataReader> HydraReader.Read ct

    /// Builds a select query with a QueryContext - returns an Async query result
    let selectAsync ct =
        selectAsync<'Selected, 'Mapped, Oracle.ManagedDataAccess.Client.OracleDataReader> HydraReader.Read ct
